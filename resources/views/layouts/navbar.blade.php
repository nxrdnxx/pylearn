...
<div id="mobile-menu" class="fixed top-16 left-0 right-0 z-[99] bg-slate-950/95 backdrop-blur-xl border-b border-gray-800 p-4 hidden lg:hidden">
    <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between gap-3 mt-4 pt-4 border-t border-gray-800">
            <div class="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-gray-900 border border-gray-800">
                <div class="w-5 h-5 rounded bg-amber-500/20 flex items-center justify-center">
                    <i class="fa-solid fa-bolt text-amber-500 text-[9px]"></i>
                </div>
                <span class="text-xs font-bold text-white">{{ number_format(auth()->user()->xp) }} XP</span>
            </div>
            <div class="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-gray-900 border border-gray-800">
                <i class="fa-solid fa-fire text-red-500 text-xs animate-pulse"></i>
                <span class="text-xs font-bold text-white">{{ auth()->user()->login_streak ?? 0 }} Hari</span>
            </div>
        </div>
        @endauth
    </div>
