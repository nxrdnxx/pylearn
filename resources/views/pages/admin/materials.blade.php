        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-white/5">
                    ...
                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-white/5">Nama Materi</th>
                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-white/5">Deskripsi</th>
                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-white/5">Status Quiz</th>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td class="px-6 py-5">
                        <div class="text-xs text-slate-500 max-w-[300px] line-clamp-2 leading-relaxed">{{ $level->description ?? '-' }}</div>
                    </td>
                    <td class="px-6 py-5">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Soal Tersedia</span>
                        </div>
                    </td>
                    <td class="px-6 py-5 text-right">
                        <div class="flex justify-end gap-2">
                            <button onclick="editMaterial({{ $level->id }}, '{{ addslashes($level->name) }}', '{{ addslashes($level->description) }}', {{ $level->order_number }})" 
                            ...
                            <form action="{{ route('admin.materials.destroy', $level) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-8 h-8 rounded-lg bg-red-600/10 text-red-400 hover:bg-red-600 hover:text-white transition-all" onclick="return confirm('Yakin hapus materi ini?')">
...
<script>
function editMaterial(id, name, description, order) {
