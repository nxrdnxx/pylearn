...
<div class="pt-16 min-h-screen bg-ink-950">
    ...
    <div class="max-w-[800px] mx-auto px-7 pb-24">
        <!-- Podium -->
        <div class="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6 mb-12 items-start md:items-end">
            @php
                $podium = collect([
                ...
                <div class="relative bg-surface-1 rounded-[28px] sm:rounded-[32px] border @if($user->rank == 1) border-brand-amber/30 shadow-[0_20px_50px_rgba(232,160,34,0.15)] @else border-white/5 @endif p-5 sm:p-8 text-center overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="relative w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-3 sm:mb-4">
                            ...
                            <div class="relative w-full h-full rounded-full bg-ink-700 border-2 @if($user->rank == 1) border-brand-amber @else border-ink-600 @endif flex items-center justify-center shadow-2xl overflow-hidden">
                            ...
                            <div class="sm:hidden absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-surface-1 border-2 border-surface-1 flex items-center justify-center shadow-lg z-20">
                                <i class="fa-solid {{ $rankIcon }} text-[8px]"></i>
                            </div>
                        </div>

                        <h3 class="text-[11px] sm:text-lg font-bold text-white mb-1 truncate">{{ $user->name }}</h3>
                        ...
                        <div class="inline-flex items-center px-2.5 py-1 rounded-lg bg-white/5 text-[10px] font-medium text-text-muted">
                            Level {{ $user->level }}
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center gap-2 sm:gap-4 py-2.5 sm:py-4 px-3 sm:px-6 rounded-2xl transition-all duration-300 hover:bg-white/[0.02] group @if($user->id == $me->id) bg-brand-blue/10 border border-brand-blue/20 @endif">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="text-[11px] sm:text-sm font-bold text-white truncate flex items-center gap-1 sm:gap-2">
                                {{ $user->name }}
                                @if($user->id == $me->id)
                                    <span class="px-2.5 py-1 rounded-lg bg-brand-blue/20 text-brand-blue-light text-[8px] sm:text-[9px] font-bold uppercase tracking-tighter">Kamu</span>
                                @endif
                            </div>
