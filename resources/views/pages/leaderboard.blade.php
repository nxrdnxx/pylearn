    ...
    <div class="max-w-[800px] mx-auto px-7 pb-24">
            ...
            <div class="podium-card group relative {{ $orderClass }} @if($user->rank == 1) md:scale-110 md:-translate-y-4 z-20 @endif transition-all duration-500">
                <div class="relative bg-surface-1 rounded-[32px] border @if($user->rank == 1) border-brand-amber/30 shadow-[0_20px_50px_rgba(232,160,34,0.15)] @else border-white/5 @endif p-8 text-center overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <div class="mb-4">
                            @if($user->rank == 1) 
                                <div class="w-14 h-14 mx-auto mb-2 flex items-center justify-center">
                                    <i class="fa-solid fa-crown text-brand-amber text-4xl drop-shadow-[0_0_15px_rgba(232,160,34,0.5)] animate-bounce-subtle"></i>
                                </div>
                            @elseif($user->rank == 2) 
                                <i class="fa-solid fa-medal text-gray-400 text-3xl mb-3"></i>
                            @else 
                                <i class="fa-solid fa-medal text-[#cd7f32] text-3xl mb-3"></i>
                            @endif
                        </div>

                        <div class="relative w-20 h-20 mx-auto mb-4">
                            <div class="absolute inset-0 bg-gradient-to-br from-brand-blue to-brand-purple rounded-full blur-lg opacity-40 group-hover:opacity-60 transition-opacity"></div>
                            <div class="relative w-full h-full rounded-full bg-surface-1 border-2 @if($user->rank == 1) border-brand-amber @else border-white/10 @endif flex items-center justify-center text-2xl font-bold text-white shadow-2xl">
                                {{ strtoupper(substr($user->name, 0, 2)) }}
                            </div>
                            @if($user->rank == 1)
                                <div class="absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-brand-amber flex items-center justify-center text-white text-xs font-bold border-2 border-surface-1">1</div>
                            @endif
                        </div>
        ...
        <div class="bg-surface-1 rounded-[32px] border border-white/5 overflow-hidden shadow-2xl">
            <div class="px-8 py-5 border-b border-white/5 flex items-center justify-between">
                <h3 class="text-sm font-bold text-white uppercase tracking-widest">Peringkat Lainnya</h3>
                <span class="text-xs text-text-muted">{{ $users->count() }} Pelajar Aktif</span>
            </div>
            ...
            <div class="p-2">
                ...
                <div class="flex items-center gap-4 py-4 px-6 rounded-2xl transition-all duration-300 hover:bg-white/[0.02] group @if($user->id == $me->id) bg-brand-blue/10 border border-brand-blue/20 @endif">
                    ...
                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 group-hover:scale-110 transition-all">
                        {{ strtoupper(substr($user->name, 0, 2)) }}
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-bold text-white truncate flex items-center gap-2">
                            {{ $user->name }}
                            @if($user->id == $me->id)
                                <span class="px-1.5 py-0.5 rounded bg-brand-blue/20 text-brand-blue-light text-[9px] font-bold uppercase tracking-tighter">Kamu</span>
                            @endif
                        </div>
                        <div class="text-[10px] text-text-muted">Level {{ $user->level }}</div>
                    </div>
