...
<div class="pt-16 min-h-screen" style="background: linear-gradient(135deg, #04091a 0%, #070f26 100%);">
    <div class="max-w-[1100px] mx-auto px-7 pt-10 pb-6">
        <div class="flex items-end justify-between gap-4 flex-wrap">
            <div class="flex flex-col gap-1">
                ...
                <h1 class="text-2xl sm:text-3xl lg:text-[36px] font-semibold text-white leading-tight">{{ $hasActivity ? 'Lanjutkan dimana kamu berhenti' : 'Ayo mulai belajar sekarang' }}</h1>
            </div>
    ...
    <div class="max-w-[1100px] mx-auto px-7 pb-6">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-3.5">
            <div class="bg-slate-900 rounded-xl border border-gray-800 p-3 sm:p-4 hover:border-gray-700 hover:bg-gray-800 transition-all duration-200">
                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-blue-500/20 flex items-center justify-center mb-2 sm:mb-3">
                    <i class="fa-solid fa-bolt text-blue-400 text-base sm:text-lg"></i>
                </div>
                <div class="text-base sm:text-xl font-semibold text-white">{{ number_format($xp) }}</div>
                <div class="text-[10px] sm:text-xs text-gray-400 mt-1">Total XP</div>
            </div>
            
            <div class="bg-slate-900 rounded-xl border border-gray-800 p-3 sm:p-4 hover:border-gray-700 hover:bg-gray-800 transition-all duration-200">
                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-green-500/20 flex items-center justify-center mb-2 sm:mb-3">
                    <i class="fa-solid fa-layer-group text-green-400 text-base sm:text-lg"></i>
                </div>
                <div class="text-base sm:text-xl font-semibold text-white">{{ $completedLevel }}<span class="text-sm sm:text-lg text-gray-400">/{{ $totalLevel }}</span></div>
            ...
            <div class="bg-slate-900 rounded-xl border border-gray-800 p-3 sm:p-4 hover:border-gray-700 hover:bg-gray-800 transition-all duration-200">
                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-amber-500/20 flex items-center justify-center mb-2 sm:mb-3">
                    <i class="fa-solid fa-fire text-amber-400 text-base sm:text-lg"></i>
                </div>
                <div class="text-base sm:text-xl font-semibold text-white">{{ $streak }}</div>
                <div class="text-[10px] sm:text-xs text-gray-400 mt-1">Hari streak</div>
            </div>
            
            <div class="bg-slate-900 rounded-xl border border-gray-800 p-3 sm:p-4 hover:border-gray-700 hover:bg-gray-800 transition-all duration-200">
                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-purple-500/20 flex items-center justify-center mb-2 sm:mb-3">
                    <i class="fa-solid fa-medal text-purple-400 text-base sm:text-lg"></i>
                </div>
                <div class="text-base sm:text-xl font-semibold text-white">{{ $badgeCount }}</div>
    ...
    <div class="max-w-[1100px] mx-auto px-7 pb-14 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
        <div class="flex flex-col gap-5">
            ...
            <div class="bg-slate-800 rounded-2xl border border-gray-700 p-4 sm:p-6 hover:border-gray-600 transition-all duration-300">
                <div class="flex justify-between items-start mb-4 sm:mb-5 gap-3">
                    <div class="min-w-0">
                        ...
                        <div class="text-base sm:text-xl font-semibold text-white truncate">{{ $currentLevel->name }}</div>
                    </div>
                ...
                <div class="w-full h-2 bg-slate-900 rounded-full overflow-hidden mb-4 sm:mb-5">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    ...
                    <a href="{{ route('quiz.show', $currentLevel->id) }}" class="w-full sm:w-auto text-center px-3 py-1.5 rounded-lg bg-blue-500 text-white font-medium text-xs hover:bg-blue-400 transition-all">
                        {{ $currentAnswered == 0 ? 'Mulai' : 'Lanjutkan' }} <i class="fa-solid fa-arrow-right ml-1.5 text-xs"></i>
                    </a>
                </div>
            </div>
            @else
            <div class="bg-slate-800 rounded-3xl border border-gray-700 p-6 sm:p-12 text-center shadow-2xl relative overflow-hidden group">
                ...
                <div class="text-lg sm:text-xl font-semibold text-white mb-2">Semua level sudah selesai!</div>
                <p class="text-sm text-gray-400">Mantap! Kamu sudah menamatkan semua materi.</p>
            </div>
            @endif

            <div class="bg-slate-900 rounded-xl border border-gray-800 p-4 sm:p-5">
                <div class="flex items-center justify-between mb-4 gap-2">
                    <h3 class="text-xs sm:text-sm font-semibold text-white"><i class="fa-solid fa-clock-rotate-left mr-2"></i>Aktivitas terbaru</h3>
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-2 sm:gap-3.5 py-2.5 sm:py-3.5 {{ !$loop->last ? 'border-b border-gray-800' : '' }}">
                        <div class="w-2 h-2 rounded-full {{ $activity['type'] === 'level' ? 'bg-green-500' : 'bg-amber-500' }} flex-shrink-0"></div>
                        <div class="flex-1 min-w-0">
                            <span class="text-xs sm:text-sm font-medium text-white truncate block">{{ $activity['title'] }}</span>
            ...
            <div class="grid grid-cols-3 gap-2 sm:gap-3.5">
                <a href="{{ route('level.index') }}" class="bg-slate-900 rounded-xl border border-gray-800 p-3 sm:p-4 flex items-center gap-2 sm:gap-3 hover:border-gray-700 hover:bg-gray-800 transition-all cursor-pointer">
                    ...
                    <div class="flex flex-col min-w-0">
                        <span class="text-[10px] sm:text-sm font-semibold text-white truncate">Level</span>
                        <span class="text-[8px] sm:text-xs text-gray-400 truncate">{{ $completedLevel }}/{{ $totalLevel }} selesai</span>
                    </div>
                </a>
                <a href="{{ route('leaderboard.index') }}" class="bg-slate-900 rounded-xl border border-gray-800 p-3 sm:p-4 flex items-center gap-2 sm:gap-3 hover:border-gray-700 hover:bg-gray-800 transition-all cursor-pointer">
                    ...
                    <div class="flex flex-col min-w-0">
                        <span class="text-[10px] sm:text-sm font-semibold text-white truncate">Leaderboard</span>
                        <span class="text-[8px] sm:text-xs text-gray-400 truncate">Posisi #{{ $rank }}</span>
                    </div>
                </a>
                <a href="{{ route('profile.index') }}" class="bg-slate-900 rounded-xl border border-gray-800 p-3 sm:p-4 flex items-center gap-2 sm:gap-3 hover:border-gray-700 hover:bg-gray-800 transition-all cursor-pointer">
                    ...
                    <div class="flex flex-col min-w-0">
                        <span class="text-[10px] sm:text-sm font-semibold text-white truncate">Profil</span>
        ...
        <div class="flex flex-col gap-5">
            ...
            <div class="bg-slate-900 rounded-xl border border-gray-800 p-4 sm:p-5">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-xs sm:text-sm font-semibold text-white"><i class="fa-solid fa-ranking-star mr-2"></i>Top pelajar</h3>
                </div>
                <div class="flex flex-col">
                    @foreach($topUsers as $i => $u)
                    <div class="flex items-center gap-2 sm:gap-3 py-2 sm:py-2.5 {{ !$loop->last ? 'border-b border-gray-800' : '' }}">
                        ...
                        <span class="flex-1 text-[11px] sm:text-sm font-medium text-white truncate">{{ $u->id == auth()->id() ? 'Kamu' : $u->name }}</span>
            ...
            <div class="bg-slate-900 rounded-xl border border-gray-800 p-4 sm:p-5">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-xs sm:text-sm font-semibold text-white"><i class="fa-solid fa-medal mr-2 text-purple-400"></i>Badge terbaru</h3>
                ...
                <div class="flex flex-col gap-2 sm:gap-3">
                    ...
                    <div class="flex items-center gap-2 sm:gap-3.5 p-2 sm:p-3 rounded-xl bg-slate-800/40 border border-gray-800/40 hover:bg-slate-800 hover:border-gray-700 transition-all group">
                        <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center border border-white/5 shadow-xl group-hover:scale-105 transition-all duration-300 flex-shrink-0" style="background: linear-gradient(135deg, {{ $color }}33, {{ $color }}1a);">
                        ...
                        <div class="flex flex-col overflow-hidden min-w-0">
                            <span class="text-xs sm:text-sm font-bold text-white truncate transition-colors" style="--badge-color: {{ $color }};" onmouseenter="this.style.color=this.style.getPropertyValue('--badge-color')" onmouseleave="this.style.color=''">{{ $ub->badge->name }}</span>
            ...
            <div class="bg-slate-900 rounded-2xl border border-gray-800 p-4 sm:p-6 shadow-xl relative overflow-hidden group" id="daily-quest-container">
                    ...
                    <div class="mb-3">
                        ...
                        <div id="quest-question-area" class="{{ $dailyQuest->completed ? 'hidden' : '' }}">
                            ...
                            <button type="button" onclick="submitQuestAnswer('{{ addslashes($opt) }}')" class="quest-option-btn text-left flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-800 bg-slate-800/40 hover:border-brand-blue/50 hover:bg-slate-800 transition-all duration-300 group relative overflow-hidden">
                                <div class="w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-slate-900 border border-slate-700 flex items-center justify-center text-[10px] text-slate-500 group-hover:border-slate-500 group-hover:text-white transition-all">
                                    {{ chr(65 + $loop->index) }}
                                </div>
                                <span class="text-xs sm:text-sm text-slate-400 group-hover:text-white transition-colors relative z-10">{{ $opt }}</span>
                            </button>
                                        @endforeach
                                    </div>
                                @else
                    ...
                    shuffled.forEach((opt, idx) => {
                        ...
                        optionsHtml += `
                            <button type="button" onclick="submitQuestAnswer('${attrSafe}')" class="quest-option-btn text-left flex items-center gap-4 p-4 rounded-2xl border border-slate-800 bg-slate-800/40 hover:border-brand-blue/50 hover:bg-slate-800 hover:translate-x-1 transition-all duration-300 group relative overflow-hidden">
                                ...
                                <span class="text-sm text-slate-400 group-hover:text-white transition-colors relative z-10">${textSafe}</span>
                            </button>
                        `;
                    });
                    optionsHtml += '</div>';
